...
var (
	regChinese     = regexp.MustCompile(Reg_Chinese)
	regChineseSign = regexp.MustCompile(Reg_Chinese_Sign)
)

func IsChinese(text string) bool {
	return regChinese.MatchString(text) || regChineseSign.MatchString(text)
}
